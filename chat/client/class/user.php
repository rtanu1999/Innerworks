...
class user{
	...
	function login_check($username,$contact){
		$sql = "SELECT * FROM loginc WHERE username=:username and contact=:contact";
		$query= $this->pdo->prepare($sql);
      ...
      function insert_login_client($username,$contact){
		$sql = "insert into loginc(username,contact)values(:username,:contact)" ;
		$query= $this->pdo->prepare($sql);
		$query->bindParam(':username',$username,PDO::PARAM_STR);
		$query->bindParam(':contact',$contact,PDO::PARAM_STR);
		$query->execute();
	...
	function insert_login_details($user_id){
		$sql = "INSERT INTO login_details_customers(user_id)VALUES (:id)";
		$query= $this->pdo->prepare($sql);

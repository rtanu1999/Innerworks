...
if(isset($_POST["edu"]))
{
 ...
 ";
        ...
        <i class="fa fa-inr" ></i>' .$row['maxSalary'] .'
          ...
          <li class="list-group-item" title="Qualification" data-etracking="true" data-ecategory="job_card_start_date" onclick="javascript:fillInedu(this.innerHTML);" style="cursor:pointer;">
...
if(isset($_POST["filltype"])){
    $type=$_POST["type"];
 //  echo $sal;
    $type = preg_replace('/\s+/','',$type);
    $qry="select * from jobpost where status=1 and type='$type'";
...
if(isset($_POST["filledu"])){
    $edu=$_POST["edu"];
 //  echo $sal;
    $edu = preg_replace('/\s+/','',$edu);
    $qry="select * from jobpost where status=1 and type='$edu'";
